...
const int humerusLS_pin = 13; //Limit switch on the back of the humerus
...
AccelStepper baseStepper(AccelStepper::FULL4WIRE, 8, 6, 7, 9); // Defaults to AccelStepper::FULL4WIRE (4 pins) on 2, 3, 4, 5
AccelStepper shoulderStepper(AccelStepper::FULL4WIRE, 2, 3, 4, 5);
Servo wristServo;
...
void setup() {
  ...
  baseStepper.setMaxSpeed(80);  // Steps per second (max value set by arduino clock speed)
...
void setMotors(float angles[3]) {
  ...
  shoulderStepper.moveTo(pos * 800*4/360);

  // run in a while loop so we can stop if needed
  while(shoulderStepper.distanceToGo() != 0){
    shoulderStepper.run();

    // if limit switch triggered, stop
    if(digitalRead(humerusLS_pin) == LOW) {
  ...
  Serial.print("Writing to Bases: ");
  Serial.println(angles[0]);
  //pos = map(angles[0],0,180,-180,0);
  baseStepper.moveTo(pos * 200/360);
...
//  }
  ...
  while(1) {
    ...
    if (digitalRead(humerusLS_pin) == 0) {
      ...
      shoulderStepper.runToNewPosition(-90.0 * 800*4/360);
