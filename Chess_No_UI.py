...
engine.configure({"UCI_LimitStrength": True, "Skill Level": 20})

############################# LOAD JSON AND FIND BOARD SQUARES #########################################################################################

with open("src/boxes.json", 'r') as fp:
    sq_points = json.load(fp)
   
    # Image cropping coordinates
    TopL_Y=int(sq_points["a1"][1][1])
    TopL_X=int(sq_points["a1"][1][0])
    TopR_X=int(sq_points["a8"][2][0])
    BottomL_Y=int(sq_points["h1"][0][1])
...
while not board.is_game_over():
    ...
    if comp_move:
        try:
            ...
            #pieceType=stockfish.get_what_is_on_square(sq1)
            #print(pieceType)

            #if pieceType == "Stockfish.Piece.WHITE_KING":
            #    tallPiece=True
            # elif pieceType == "Stockfish.Piece.BLACK_KING":
            #     tallPiece=True
            # elif pieceType == "Stockfish.Piece.WHITE_QUEEN":
            #     tallPiece=True
            # elif pieceType == "Stockfish.Piece.BLACK_QUEEN":
            #     tallPiece=True

            #if tallPiece == True:
            #    print("Tall piece detected")
